    ...
    <main style={{ background: '#070714', minHeight: '100vh', color: '#fff', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '32px 24px' }}>
          ...
          return (
            <div key={subjectKey} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${color}20`, borderRadius: '20px', padding: '24px', marginBottom: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <button onClick={() => router.push(`/dashboard/chat/${subjectKey}`)} style={{ background: `${color}15`, border: `1px solid ${color}30`, color, padding: '6px 14px', borderRadius: '50px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
                  Belajar →
                </button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div style={{ background: 'rgba(255,45,120,0.05)', border: '1px solid rgba(255,45,120,0.1)', borderRadius: '12px', padding: '16px' }}>
                  ...
                  <div style={{ fontSize: '13px', color: '#ff2d78' }}>Belum dikenal pasti — teruskan belajar dengan Zed!</div>
                </div>
                <div style={{ background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.1)', borderRadius: '12px', padding: '16px' }}>
                  ...
                  <div style={{ fontSize: '13px', color: '#10b981' }}>Belum dikenal pasti — teruskan belajar dengan Zed!</div>
                </div>
              </div>
            </div>
          );
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(0,212,255,0.06), rgba(124,58,237,0.06))', border: '1px solid rgba(0,212,255,0.15)', borderRadius: '20px', padding: '24px', textAlign: 'center' }}>
          ...
          <button onClick={() => router.push('/dashboard')} style={{ marginTop: '16px', background: 'linear-gradient(135deg, #00d4ff, #7c3aed)', border: 'none', color: '#fff', padding: '12px 28px', borderRadius: '50px', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}>
            Mula Belajar →
          </button>
        </div>
