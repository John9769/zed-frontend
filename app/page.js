...
export default function LandingPage() {
  ...
  return (
    <main style={{ background: '#070714', minHeight: '100vh', color: '#fff', fontFamily: 'Inter, sans-serif', overflowX: 'hidden' }}>
      ...
      {/* NAVBAR */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: scrolled ? 'rgba(7,7,20,0.95)' : 'transparent', backdropFilter: scrolled ? 'blur(12px)' : 'none', borderBottom: scrolled ? '1px solid rgba(0,212,255,0.1)' : 'none', transition: 'all 0.3s ease' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <button onClick={() => router.push('/login')} style={{ background: 'transparent', border: '1px solid rgba(0,212,255,0.4)', color: '#00d4ff', padding: '8px 20px', borderRadius: '50px', cursor: 'pointer', fontSize: '14px', fontWeight: 600 }}>Login</button>
          <button onClick={() => router.push('/register')} style={{ background: 'linear-gradient(135deg, #00d4ff, #7c3aed)', border: 'none', color: '#fff', padding: '8px 20px', borderRadius: '50px', cursor: 'pointer', fontSize: '14px', fontWeight: 700, boxShadow: '0 0 20px rgba(0,212,255,0.4)' }}>Join Zed</button>
        </div>
      </nav>

      {/* HERO */}
      <section style={{ minHeight: '100vh', padding: '100px 24px 80px', background: 'radial-gradient(ellipse at top, #0d0d2b 0%, #070714 60%), radial-gradient(ellipse at bottom right, #1a0533 0%, transparent 60%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '60px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: '280px' }}>
              <div style={{ display: 'inline-block', background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.3)', borderRadius: '50px', padding: '6px 16px', fontSize: '12px', color: '#00d4ff', fontWeight: 600, marginBottom: '20px', letterSpacing: '2px' }}>
              ...
              {/* MOBILE VIDEO — hidden on desktop via CSS */}
              <div className="mobile-video" style={{ marginBottom: '24px' }}>
                <div style={{ position: 'relative', width: '100%', maxWidth: '360px', height: '380px', borderRadius: '20px', overflow: 'hidden', border: '1px solid rgba(0,212,255,0.2)', boxShadow: '0 0 40px rgba(0,212,255,0.15)' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '100px', background: 'linear-gradient(transparent, rgba(7,7,20,0.9))' }} />
              ...
              <p style={{ fontSize: '17px', color: '#94a3b8', lineHeight: 1.7, marginBottom: '16px', maxWidth: '480px' }}>
                Zed bukan sekadar AI tutor. Dia memahami bila anda penat, malas, atau stress — dan tetap ada untuk anda. 24/7. Tak pernah marah.
              </p>
              <p style={{ fontSize: '15px', color: '#00d4ff', marginBottom: '32px', fontWeight: 600 }}>
                ✦ Belajar SPM • Dapat duit balik • Tabung masa depan
              </p>

              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                <button onClick={() => router.push('/register')} style={{ background: 'linear-gradient(135deg, #00d4ff, #7c3aed)', border: 'none', color: '#fff', padding: '14px 32px', borderRadius: '50px', fontSize: '15px', fontWeight: 700, cursor: 'pointer', boxShadow: '0 0 30px rgba(0,212,255,0.4)' }}>
                  Daftar Sekarang →
                </button>
                <button onClick={() => document.getElementById('pricing').scrollIntoView({ behavior: 'smooth' })} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', padding: '14px 32px', borderRadius: '50px', fontSize: '15px', fontWeight: 600, cursor: 'pointer' }}>
                  Tengok Harga
                </button>
              </div>
              ...
              <div style={{ marginTop: '40px', display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
                {[
                  ...
                  <div key={i}>
                    <div style={{ fontSize: '22px', fontWeight: 900, color: '#00d4ff' }}>{stat.number}</div>
            ...
            <div className="desktop-video" style={{ flex: 1, minWidth: '280px', justifyContent: 'center', position: 'relative' }}>
              <div style={{ position: 'relative', width: '100%', maxWidth: '420px', height: '520px', borderRadius: '24px', overflow: 'hidden', border: '1px solid rgba(0,212,255,0.2)', boxShadow: '0 0 60px rgba(0,212,255,0.2), 0 0 120px rgba(124,58,237,0.15)' }}>
                <video autoPlay muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }} poster="/assets/Zed.png">
                  <source src="/assets/Zed.mp4" type="video/mp4" />
                </video>
              ...
              <div style={{ position: 'absolute', top: '20px', right: '0px', background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.3)', borderRadius: '12px', padding: '12px 16px', backdropFilter: 'blur(12px)' }}>
              ...
              <div style={{ position: 'absolute', bottom: '60px', left: '0px', background: 'rgba(124,58,237,0.15)', border: '1px solid rgba(124,58,237,0.3)', borderRadius: '12px', padding: '12px 16px', backdropFilter: 'blur(12px)' }}>
                <div style={{ fontSize: '20px', marginBottom: '4px' }}>💰</div>
      ...
      <section style={{ padding: '100px 24px', position: 'relative' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '64px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 900 }}>
              Simple je.<br />
              <span style={{ background: 'linear-gradient(135deg, #00d4ff, #7c3aed)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>4 langkah.</span>
            </h2>
          </div>
      ...
      {/* ZED FUND */}
      <section style={{ padding: '100px 24px', background: 'radial-gradient(ellipse at center, #0d0533 0%, #070714 70%)' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(24px, 4vw, 48px)', fontWeight: 900, marginBottom: '24px', lineHeight: 1.3 }}>
            Belajar SPM.<br />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '16px', marginBottom: '48px' }}>
            {[
              { label: 'Setiap subjek', credit: 'RM5/bulan', color: '#00d4ff' },
              { label: 'Kawan 3 subjek', credit: 'RM15/bulan', color: '#7c3aed' },
              { label: '10 kawan × 3', credit: 'RM150/bulan', color: '#ff2d78' }
            ].map((item, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${item.color}30`, borderRadius: '16px', padding: '20px 12px' }}>
                <div style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '8px' }}>{item.label}</div>
                <div style={{ fontSize: '16px', fontWeight: 800, color: item.color }}>{item.credit}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="pricing" style={{ padding: '100px 24px' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '64px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(20px, 3vw, 40px)', fontWeight: 900 }}>
              Tuisyen 1 subjek = RM100+/bulan.{' '}
              <span style={{ background: 'linear-gradient(135deg, #00d4ff, #7c3aed)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Zed?</span>
            </h2>
          </div>
          <div style={{ maxWidth: '420px', margin: '0 auto' }}>
            <div style={{ background: 'rgba(0,212,255,0.05)', border: '1px solid rgba(0,212,255,0.3)', borderRadius: '24px', padding: '40px 28px', textAlign: 'center', position: 'relative' }}>
              <div style={{ position: 'absolute', top: '-14px', left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg, #00d4ff, #7c3aed)', padding: '4px 20px', borderRadius: '50px', fontSize: '11px', fontWeight: 700, whiteSpace: 'nowrap' }}>
                🎯 EARLY BIRD — TERHAD 10,000 PELAJAR PERTAMA
              </div>
              <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '8px' }}>Harga per subjek</div>
              <div style={{ fontSize: '44px', fontWeight: 900, color: '#00d4ff', lineHeight: 1, marginBottom: '4px' }}>RM19.99</div>
              <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '8px' }}>/bulan • Early Bird</div>
              <div style={{ fontSize: '13px', color: '#475569', textDecoration: 'line-through', marginBottom: '28px' }}>Harga biasa RM29.99/bulan</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '32px', textAlign: 'left' }}>
                {[
                  ...
                  <div key={i} style={{ fontSize: '13px', color: '#94a3b8' }}>{f}</div>
                ))}
              </div>
              <button onClick={() => router.push('/register')} style={{ width: '100%', background: 'linear-gradient(135deg, #00d4ff, #7c3aed)', border: 'none', color: '#fff', padding: '14px', borderRadius: '12px', fontSize: '15px', fontWeight: 700, cursor: 'pointer', boxShadow: '0 0 20px rgba(0,212,255,0.3)' }}>
                Cuba Zed Percuma →
              </button>
